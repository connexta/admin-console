...
class CreateServiceDependencyGraphTest extends Specification {

    private BundleUtils bundleUtils = Mock(BundleUtils)
    private CreateServiceDependencyGraph function = new CreateServiceDependencyGraph(bundleUtils)
    private List<String> FUNCTION_PATH = [CreateServiceDependencyGraph.FUNCTION_NAME]
    ...
    def setup() {
        ...
        defaultSavePath = Paths.get(tempSaveDir.getAbsolutePath(), CreateServiceDependencyGraph.DEFAULT_GRAPH_NAME).toFile()
    ...
    def "Graph generates with bundles that can't be found"() {
        ...
        bundleContainingUnknownId.serviceRefs().forEach({ ref -> ref.service().bundleId(-1) })
    ...
    def "Error returned when graph creation fails"() {
        setup:
        bundleUtils.getAllBundleFields() >> [SampleData.bundleA(), SampleData.bundleB(), new BundleField()]
    ...
    def "Validate arguments correctly"() {
        when:
        Report<Boolean> report = function.execute([(CreateServiceDependencyGraph.SAVE_DIR): Paths.get("does", "not", "exist").toString()], FUNCTION_PATH)

        then:
        report.getErrorMessages().size() == 1
        report.getErrorMessages().get(0).getCode() == DefaultMessages.DIRECTORY_DOES_NOT_EXIST
        report.getErrorMessages().get(0).getPath() == FUNCTION_PATH + CreateServiceDependencyGraph.SAVE_DIR
    }
    ...
    def "Use save dir when specified"() {
        ...
        def newGraphLocation = Paths.get(newSaveDir.getAbsolutePath(), CreateServiceDependencyGraph.DEFAULT_GRAPH_NAME).toFile()

        when:
        Report<Boolean> report = function.execute([(CreateServiceDependencyGraph.SAVE_DIR): newSaveDir.getAbsolutePath()], FUNCTION_PATH)
