...
public class GraphQLTransformOutput {
  ...
  public GraphQLOutputType fieldToGraphQLOutputType(DataType field) {
    ...
    if (field instanceof ObjectField) {
      ...
      try {
        type =
            new GraphQLList(
                fieldToGraphQLOutputType(((ListField<DataType>) field).createListEntry()));
      } catch (Exception e) {
        throw new RuntimeException(
            "Unable to build field list content type for output type: " + field.fieldName());
      }
  ...
  public GraphQLFieldDefinition fieldToGraphQLFieldDefinition(Field field) {
    ...
    return GraphQLFieldDefinition.newFieldDefinition()
        ...
        .dataFetcher(
            field instanceof FunctionField
                ? (env -> functionDataFetcher(env, (FunctionField) field))
                : (env -> dataTypeDataFetcher(env, (DataType) field)))
        .build();
  }
  ...
  public Object functionDataFetcher(DataFetchingEnvironment env, FunctionField<DataType> field) {
    ...
    if (!result.messages().isEmpty()) {
      throw new FunctionDataFetcherException(
          funcField.fieldName(),
          funcField.getArguments().stream().map(Field::getValue).collect(Collectors.toList()),
          result.messages());
