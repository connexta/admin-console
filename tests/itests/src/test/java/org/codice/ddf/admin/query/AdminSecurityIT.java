...
public class AdminSecurityIT extends AbstractComponentTest {
    ...
    public static final WcpmRequestHelper WCPM_REQUEST_HELPER = new WcpmRequestHelper(
            GRAPHQL_ENDPOINT);

    public static final LdapRequestHelper LDAP_REQUEST_HELPER = new LdapRequestHelper(
            GRAPHQL_ENDPOINT);
    ...
    public void saveLdapAuthenticationConfig() throws IOException {
        try {
            LdapConfigurationField newConfig =
                    createSampleLdapConfiguration(LdapUseCase.AUTHENTICATION);
            LDAP_REQUEST_HELPER.createLdapConfig(newConfig);
            LDAP_REQUEST_HELPER.waitForConfigs(Collections.singletonList(newConfig.getValue()),
                    true);
        } finally {
    ...
    public void saveLdapAttributeStoreConfig() throws IOException {
        try {
            LdapConfigurationField newConfig =
                    createSampleLdapConfiguration(LdapUseCase.ATTRIBUTE_STORE);
            LDAP_REQUEST_HELPER.createLdapConfig(newConfig);
            LDAP_REQUEST_HELPER.waitForConfigs(Collections.singletonList(newConfig.getValue()),
                    true);
        } finally {
    ...
    public void saveLdapAuthenticationAndAttributeStoreConfig() throws IOException {
        try {
            ...
            List<Map<String, Object>> expectedConfigs = ImmutableList.of(
    ...
    public LdapConfigurationField createSampleLdapConfiguration(EnumValue<String> ldapUseCase) {
        ...
        LdapDirectorySettingsField dirSettings =
                new LdapDirectorySettingsField().baseUserDn(TEST_DN)
        ...
        if (ldapUseCase.getValue()
            ...
            dirSettings.groupAttributeHoldingMember(TEST_ATTRIBUTE)
