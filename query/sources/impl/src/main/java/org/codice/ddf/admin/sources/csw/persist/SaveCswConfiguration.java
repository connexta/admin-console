...
import static org.codice.ddf.admin.sources.commons.services.CswServiceProperties.cswConfigToServiceProps;
...
public class SaveCswConfiguration extends BaseAction<SourceInfoField> {
    ...
    private SourceInfoField updateExistingConfig() {
        List<Message> results = updateConfig(servicePid, config, configuratorFactory,
    ...
    private SourceInfoField persistNewConfig() {
        List<Message> results = persist(config, configuratorFactory,
                cswConfigToServiceProps(config));
