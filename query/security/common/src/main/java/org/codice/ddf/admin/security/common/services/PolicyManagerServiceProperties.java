...
public class PolicyManagerServiceProperties {
    ...
    for (ContextPolicy policy : allPolicies) {
      ...
      for (ContextPolicyBin bin : policies) {
        if (bin.realm().equals(policy.getRealm())
            ...
            && hasSameRequiredAttributes(bin, policyRequiredAttributes)) {
  ...
  private boolean hasSameRequiredAttributes(
      ContextPolicyBin bin, Map<String, String> mappingsToCheck) {

    if (!(bin.claimsMapping().keySet().containsAll(mappingsToCheck.keySet())
        && mappingsToCheck.keySet().containsAll(bin.claimsMapping().keySet()))) {
      return false;
    }
    ...
    return !bin.claimsMapping()
        ...
        .stream()
        .filter(
            binMapping -> !mappingsToCheck.get(binMapping.getKey()).equals(binMapping.getValue()))
        .findFirst()
        .isPresent();
  }
