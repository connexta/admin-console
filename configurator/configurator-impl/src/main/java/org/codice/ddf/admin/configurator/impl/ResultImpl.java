...
public class ResultImpl<T> implements Result<T> {
    ...
    private ResultImpl(Status status, Throwable error, T data) {
        this.status = status;
        this.error = error;
        this.data = data;
    }
    ...
    static Result<Void> fail(Throwable throwable) {
    ...
    static Result<Void> rollbackFail(Throwable throwable) {
        return new ResultImpl<>(Status.ROLLBACK_FAILED, throwable, null);
    }

    static <S> Result<S> rollbackFailWithData(Throwable throwable, S data) {
        return new ResultImpl<>(Status.ROLLBACK_FAILED, throwable, data);
    }
