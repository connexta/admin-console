...
import static org.codice.ddf.admin.ldap.fields.connection.LdapBindMethod.SimpleEnumValue.SIMPLE;
...
public class LdapTestingUtils {
   ...
   *
   * <p>Possible message types: FAILED_TEST_SETUP, CANNOT_CONNECT, FAILED_TEST_SETUP
   *
   * @return connection attempt wrapper object
   */
  @SuppressWarnings("squid:S2095" /* Method returns an open LdapConnectionAttempt */)
  public LdapConnectionAttempt getLdapConnection(LdapConnectionField connection) {
    ...
    try {
      if (connection.encryptionMethod().equals(LDAPS) //
          || connection.encryptionMethod().equals(START_TLS)) {
        ldapOptions.set(LDAPConnectionFactory.SSL_CONTEXT, getSslContext());

        if (connection.encryptionMethod().equals(START_TLS)) {
      ...
      ldapOptions.set(
          ...
          Arrays.asList(System.getProperty(CIPHER_SUITES_SYS_PROP).split(",")));
      ldapOptions.set(
    ...
    try (LDAPConnectionFactory connectionFactory =
        new LDAPConnectionFactory(connection.hostname(), connection.port(), ldapOptions)) {
   ...
   *
   * <p>Possible message types: FAILED_TEST_SETUP, CANNOT_CONNECT, CANNOT_BIND
   *
   * @param connField
   * @param bindInfo
   * @return
  ...
  public LdapConnectionAttempt bindUserToLdapConnection(
    ...
    try {
      BindRequest bindRequest =
          selectBindMethod(
              bindInfo.bindMethod(),
              bindInfo.credentialsField().username(),
              bindInfo.credentialsField().password(),
              bindInfo.realm(),
              null);
   ...
   */
  public List<SearchResultEntry> getLdapQueryResults(
      Connection ldapConnection,
  ...
  /**
   * Checks for the provided DN. If it does not exist, the {@code Optional} contains the path to the
   * DN; else, an empty {@code Optional}.
   *
   * @param dirDn the DN to check
   * @param ldapConnection the connection to the LDAP server to use
   * @return If the path does not exist, the {@code Optional} contains the path to the DN; else, an
   *     empty {@code Optional}.
   */
  public Report<Void> checkDirExists(LdapDistinguishedName dirDn, Connection ldapConnection) {
    boolean dirExists =
        !getLdapQueryResults(
                ldapConnection,
                dirDn.getValue(),
                Filter.present("objectClass").toString(),
