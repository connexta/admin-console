...
public class TestFieldProvider extends BaseFieldProvider {
  ...
  public List<Field> getDiscoveryFields() {
    return ImmutableList.of(
        getInt, getBoolean, getString, getList, getEnum, errorsPath, multiArgFunc, reqArgFunc);
  }
  ...
  public static class ReturnErrorsFunction extends BaseFunctionField<TestObjectField> {
    ...
    public TestObjectField performFunction() {
      ...
      addArgumentMessage(
          sampleArgumentError(objectFieldArg.getListField().getList().get(0).path()));
    ...
    @Override
    public Set<String> getFunctionErrorCodes() {
      return new ImmutableSet.Builder<String>().add(ARGUMENT_MSG).add(RETURN_VALUE_MSG).build();
    }
  }
